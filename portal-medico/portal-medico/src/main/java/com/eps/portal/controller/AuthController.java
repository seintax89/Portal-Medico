...
public class AuthController {
    ...
    public ResponseEntity<?> iniciarSesion(@Valid @RequestBody LoginRequest loginRequest) {
        try {
            ...
            // 2. Si es correcto, guardamos la sesión
            ...
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body(new MensajeResponse("Credenciales incorrectas. Verifique su correo y contraseña."));
        }
    }
    ...
    public ResponseEntity<?> registrarMedico(@Valid @RequestBody RegistroMedicoRequest request) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(new MensajeResponse("Médico registrado exitosamente"));
    }
}